...
![image](https://github.com/user-attachments/assets/edacc9e0-b136-4808-99ae-fca8e0c7a05d)


| Таблица | Описание         |
|---------|------------------------------------------------------------------------|
| User    | User хранит данные пользователей  |
...
| Channel    | Табаблица данных о канале |
...
channel_id: 32 байт
sender_id: 32 байт
encrypted_data: 2080 байт
status: 64 байт
created_at: 8 байт
updated_at: 8 байт
encryption_key_id: 32 байт
ttl: 64 байт
Итого: 3232 байт
...
encrypted_data:
- Хеш сообщения: 32 байта (для SHA-256)
- Заголовок шифрования: 16 байт (для AES-256-GCM)
- Padded data: 16 байт (минимальная защита от padding oracle attacks)
- Плагин MTProto: 32 байта (предполагается для дополнительных данных)
...
- Все поля, являющиеся ключами в таблицах (id, username, media_id и т.д.), должны быть уникальными.
- При создании или удалении каналов, необходимо синхронизировать данные во всех связанных таблицах (Channel, Channel_subscriptions, Message).
...
- Также при создании новой записи в таблице, где есть created_at - должен срабатывать триггер для согласованности.
...
Для снижения нагрузки было добавлено поле last_message_id в таблицы к чатам и каналам, так как при открытии страницы каналов/чатов часто запрашивается опследнее сообщение.
