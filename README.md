...
3. CDN: используется для кэширования(аватарок, так как они постоянно запрашиваются); Nginx: работает как L7 балансировщик и прокси-сервер, то есть распределяет запросы между различными сервисами Telegram. CDN/Nginx распределит запросы между различными сервисами внутри кластера.
4. Service Discovery поможет Nginx определить доступные сервисы.
5. Kubernetes-кластер содержит контейнеризованные версии различных сервисов Telegram.
...
2. Kubernetes использует Service Discovery для определения доступных сервисов и при отказе одного сервера другие продолжают работать, а клиент автоматически направляет запросы на работающие.
3. Nginx распределяет нагрузку между оставшимися серверами, чтобы снизить влияние отказа одного из них на пользователя.
4. Для мониторинга можно использовать Grafana.
