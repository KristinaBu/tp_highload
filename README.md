...
Часть, связанная с криптографией, подразделяется на 3 модуля: авторизацию( работает во время первого запуска приложения, получает ключ авторизации), ключ шифрования(устанавливает секретный ключ между клиентами, сквозное шифрование) и шифрование сообщения (шифрует сообщения симметричным AES). 
Вот сам алгоритм:
...
|---------|----------|----------------|
| Redis    | Хранение сессий | Высокая производительность, работа в режиме in-memory. Быстрый доступ к данным сессий, возможность сохранения данных на диске, что обеспечивает отказоустойчивость и возможность восстановления данных после перезагрузки сервера |
| Cassandra    | Основная бд | Использует распределенную архитектуру, что позволяет легко горизонтально масштабировать систему. Cassandra также эффективна для обработки большого количества запросов на запись, что хорошо для мессенджера, засчет ассинхронной работы |
| Nginx    | Балансировка нагрузки и прокси-сервер |  Nginx имеет высокую производительность и способен обрабатывать большое количество соединений параллельно, эффективно распределяет запросы между несколькими серверами, минимизируя перегрузку любого одного сервера, предоставляет возможности для обработки HTTP-запросов, включая SSL/TLS шифрование, что критично для безопасности |
| Kubernetes    | Контейнеризирование сервисов | Kubernetes осуществляет контейнеризацию и оркестрацию сервисов, обеспечивает автоматическое масштабирование, обновление и восстановление при сбоях. Kubernetes также предоставляет инструменты для управления конфигурацией, мониторинга и логирования, что значительно упрощает управление сложными системами. Кроме того, он поддерживает различные контейнерные образы, включая Docker, это обеспечивает гибкость в выборе технологий разработки. |
| Golang    | Язык разработки | Golang (Go) - отличным выбором для разработки высокопроизводительных систем благодаря своей синхронной архитектуре и эффективной многопоточности. Он обеспечивает высокую скорость компиляции и быструю сборку исполняемого файла, что значительно упрощает процесс разработки и тестирования, предоставляет мощные инструменты для работы с сетевыми протоколами и параллельной обработкой данных, подходит для разработки микросервисной архитектуры |
| S3    | Хранение аватарок | Надежное и  масштабируемое облачное хранилище, которое подходит для хранения статических файлов, таких как аватарки пользователей. S3 обеспечивает высокую доступность и отказоустойчивость. Кроме того, он предоставляет гибкие возможности для управления правами доступа к данным (безопасность) |
| QT    | Интерфейс | QT позволяет создавать приложения, которые будут работать на различных операционных системах (Windows, macOS, Linux, мобильные устройства), ускоряет процесс разработки благодаря готовым компонентам. Также имеет поддержку многих яп |

## 9. Обеспечение надежности

### Redis

Для отказоустойчивости в Redis можно использовать Redis Sentinel. Он использует отдельный следящий узел для мониторинга доступности мастера и настраивает реплики если мастер выйдет из строя. Sentiel определяет slave на роль master и на ходу осуществит перенастройку. На slave- и master- узлах настраивается сервис Redis Sentinel, который работает таким образом [^16], [^17]:    
- проверяет доступность master и slave и отправляет сообщения, если они недоступны
- при отказе master, sentinel узел переводит slave в режим master и перенастроит другие slave так, чтобы они работали с новым master
- может вносить при необходимости изменения в конфиги узлов

То есть происходит автоматическое управление failover процессом.

### Cassandra

Для бд нужно применить шардирование (как указано в пункте 6), чтобы данные были распределены по узлам для снижения нагрузки на узлы - обеспечение отказоустойчивости.

Кроме того, при отказе бд данные могут восстановиться через журнал коммитов. Также Cassandra поддерживает 3 механизма восстановления даных на протяжении работы:
1. Чтение с восстановлением (*read repair*) - согласованность при каждом чтении. То есть во время чтения данные запрашиваются со всех реплик и сравниваются. Если на колонке стоит последняя метка врммени, то она считается верной и устаревшие записи переписываются на данную.
2. Направленная отправка (*hinted handoff*) - данные будут записаны, даже если узел-реплика недоступен. То есть если координатор-узел не может сейчас отправить данные на недоступный узел, то он записывает их в hint, и когда узел восстанавливается, то сразу записывает себе данные из hint. Потом все хинты удаляются. Можно обеспечить полную лоступность absolute write availability при включенном режиме согласованности any (то есть если запись записалась хоть на 1 кластер, то операция считается успешной), так как данные сохраняются на узле-координаторе.   
3. Анти-энтропийное восстановление узла (anti-entropy repair) - позволяет периодически проверять и восстанавливать данные на всех узлах, обеспечивая их согласованность. Тогда при отказе узла можно не волноваться, так как данные на остальных узлах будут актуальными.
...
![image](https://github.com/user-attachments/assets/5dee5472-f2a2-493d-8eb1-1fcacd6be257)


Составляющие схемы:   
| Элемент | Роль |
|---|---|
| Пользователь | Источник запросов, взаимодействует с системой через пользовательский интерфейс. |
| Latency-based DNS | Определяет оптимальный сервер для обработки запроса на основе времени отклика. |
| NGINX | Обратный прокси-сервер, принимает входящие запросы, маршрутизирует их к соответствующим микросервисам, обеспечивает балансировку нагрузки. |
| CDN | Сеть доставки контента, кэширует статические файлы (изображения, CSS, JS) для ускорения загрузки страниц и снижения нагрузки на серверы. |
| S3 | Объектное хранилище для изображений. |
| Auth | Микросервис аутентификации и авторизации пользователей, управляет сессиями. |
| Profile | Микросервис для хранения и управления профилями пользователей. |
| Media | Микросервис для работы с изображениями, сохранения их в S3. |
| MTProto | Микросервис для шифрования сообщений. |
| Chat | Микросервис для обработки сообщений, чатов и каналов. |
| Search | Микросервис для осуществления поиска информации по сообщениям, чатам, каналам. |
| Cassandra | Основная БД. |
| Redis | Используется для хранения сессий. |

Когда пользователь вводит URL страницы в браузер, Latency-based DNS определяет ближайший сервер с наименьшим временем отклика. Браузер отправляет запрос на Nginx. Nginx маршрутизирует запрос на микросервис. Если есть изображения, то выполняется запрос на CDN для их загрузки, тогда CDN отдает изображения из кэша или запрашивает их у S3 и кэширует.

Вот примеры запросов:

Регистрация:   
1. Пользователь вводит свой номер телефона.
2. Отправляется запрос в Auth.
3. Данные пользователя сверяются с данными в Cassandra.
4. Auth генерирует сессию в Redis.
5. MTProto сохраняет auth_key, связанный с конкретным user_id и сохраняет его в Cassandra.

Авторизация:
1. Пользователь вводит номер телефона и пароль.
2. Клиент отправляет запрос на Auth.
3. Auth проверяет существование пользователя в Cassandra, сверяет введенный пароль с хэшированным паролем из бд.
4. Генерирует новую сессию в Redis (если старой нет или она истекла).

Установка аватарки:
1. Пользователь выбирает изображение и отправляет его.
2. Profile проверяет авторизацию пользователя и передает данные в Media.
3. Media сохраняет изображение в S3 и получает URL.
4. Profile сохраняет полученный URL как аватарку у конкретного user_id.

Отправка сообщения: 
1. Пользователь вводит текст сообщения.
2. Запрос на отправку сообщения направляется в Chat.
3. Оттуда в MTProto поступает запрос на шифрование сообщения после проверки валидности сессии в Auth.
4. Используя auth_key пользователя и пакет шифрования из запроса, сообщение шифруется. Chat получает конечный вид сообщения, кладет его в бд.

Подписка на канал: 
1. Пользователь нажимает на кнопку "подписаться"
2. Запрос отправляется в Chat.
3. Проверяется валидность сессии в Auth.
4. user_id и channel_id добавляются в таблицу подписчиков. 

Поиск: 
1. Пользователь вводит текст в поле для поиска.
2. Запрос поступает на Search.
3. Из бд извлекается подходящий результат, его получает Search.
4. Результат возвращается пользователю.

## 11. Список серверов

Всего 15 ЦОДов (см. пункт "Глобальная балансировка нагрузки").   
Для 1 ЦОДа:   

| Сервис | RPS нагрузка | CPU | RAM Гб | Трафик, Гбит/с |
|---|---|---|---|---|
| Auth | 1 010  | 20 | 4 | 0,004088 |
| Profile | 953 | 10 | 2 | 0,000988 |
| Media | 85 | 1 | 1 | 0.0183 |
| MTProto | 154 258 | 155 | 308,5 | 1.38 |
| Chat | 204 165  | 205 | 408 | 1.82 |
| Search | 76 | 1 | 2 | 0.000155 |

Данные RPS ориентированы на пункт "Расчет нагрузки", взяты пиковые значения.   
CPU: Для логики средней сложности для Golang взято 100 RPS / 1 CPU.     
Также взято 2 ГБ RAM на 1000 запросов.     
Для media учитываются запросы не только для аватарок пользователя, но и для аватарок каналов, поэтому принималось, что при создании канала автоматически задается аватарка канала.   
Для Chat учитываются запросы сообщений, создания каналов.   
Для MTProto учитываются запросы для шифрования сообщений.     
Для Search трудно оценить размер ответа, поэтому принято, что в среднем это 1 Кб.
Для трафика учитываются данные из пункта "Логическая схема БД" и из таблицы конфигураций. 

| БД | RPS нагрузка | CPU | Суммарно| RAM | Disk |
|---|---|---|---|---|---|
| Cassandra | 205 279 | 206 | 490 Тб | 120 - 150 Гб | 500 Тб |
| Redis | 1 010 | 11 | 38 Гб | 32 - 50 Гб | 50 Гб |
| S3 | 85 | 5 | 475 Тб | 16 - 32 Гб | 500 Гб |

БД:   
Данные для БД Cassandra взяты из пукта "Физическая схема БД". Для Cassandra учитывались запросы создания пользователей, каналов, сообщений и чтения сообщений в момент их написания, запросы на чтение в бд при поиске.     
Я рассчитываю, что в среднем запись в Redis не будет превышать 4 Кб.     
В S3 хранятся аватарки, примерный размер аватарки взят из пункта "Расчет нагрузки".  

Оркестрация для *K8s*.      
Всего 15 ЦОД. Нагрузка по сервисам:   
| Сервис | % нагрузки |
|---|---|
| Auth | 0,28%  |
| Profile | 0,26% |
...
Поэтому приблизительно такое распределение контецнеров по сервисам:
| Сервис | Число контейнеров |
|---|---|
| Nginx | 15  |
| Auth | 3  |
| Profile | 3 |
...
| Сервис	| CPU/req	| CPU/lim |	RAM/req	| RAM/lim | Cnt | 
|---|---|---|---|---|---|
| Auth | 11 | 20 | 2024 | 2200 | 3 |
| Profile | 10 | 13 | 520 | 800 | 3 |
| Media | 1 | 3 | 32 | 100 | 3 |
| MTProto| 155 | 160 | 2064 | 2500 | 15 |
| Chat |205 | 215 | 2312 | 3000 | 15 |
| Search | 1 | 2 | 1000 | 1500 | 1 |

Аренда:

| Сервис	| Конфигурация	 | Аренда |
|---|---|---|
| Kube node	| 1x64GB/1xNVMe256GB/10Gb/s | $298 |
| Redis	|7302P 1x32GB/1xNVMe256G/2x10Gb/s | 	$500 |
| Cassandra	| 2х6338 16x32GB/1xNVMe4T/25Gb/s	 | $2 000 |
| S3	| 2х6338 16x32GB/1xNVMe4T/25Gb/s	 | $2 000 |
