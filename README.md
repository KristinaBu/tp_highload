...
1. Чтение с восстановлением (*read repair*) - согласованность при каждом чтении. То есть во время чтения данные запроашиваются со всех реплик и сравниваются. Если на колонке стоит последняя метка врммени, то она считается верной и устаревшие записи переписываются на данную.
...
Мониторинг, логирование с помощью Grafana, Prometeus(``` prometheus/client_golang ```) помогут своевременно получать информацию о проблемах.      
...
AWS S3 может создавать несколько версий одного и того же объекта. Каждая версия сохраняется независимо, так что можно легко восстановить предыдущие версии данных при необходимости. Также есть Lifecycle policies: настройка политики жизненного цикла для автоматического удаления старых версий.             
То есть версионирование создает множество версий объектов, а политики жизненного цикла позволяют управлять этими версиями, удаляя старые и перемещая актуальные в подходящие классы хранения.
